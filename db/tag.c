...
void add_tag_schema(const char *table, struct schema *schema)
{
    ...
    tag = hash_find(gbl_tag_hash, &table);
    if (tag == NULL) {
...
void del_tag_schema(const char *table, const char *tagname)
{
    ...
    struct dbtag *tag = hash_find(gbl_tag_hash, &table);
    if (tag == NULL) {
        unlock_taglock();
...
void debug_dump_tags(const char *tblname)
{
    ...
    tag = hash_find(gbl_tag_hash, &tblname);
    if (tag == NULL) {
        unlock_taglock();
        return;
    }
        ...
        for (i = 0; i < count; i++) {
            if (s[i]->ix) {
                ...
                for (j = 0; j < s[i]->nix; j++) {
                    ...
                    if (k == count) {
                        if (!hash_find(dbt->tags, &s[i]->ix[j]->tag))
                            freeschema(s[i]->ix[j]);
                    }
                }
                free(s[i]->ix);
...
void replace_tag_schema(dbtable *db, struct schema *schema)
{
    ...
    lock_taglock();
    dbt = hash_find(gbl_tag_hash, &db->tablename);
    if (dbt == NULL) {
        unlock_taglock();
        return;
    }
