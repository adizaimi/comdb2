    ...
    for (int ixnum = 0; ixnum < iq->usedb->nix; ixnum++) {
        ...
        if (keysize < 0) {
            ...
            reqerrstr(iq, COMDB2_ADD_RC_INVL_KEY,
                      "bad index %d or keylength %d", ixnum, keysize);
            *ixfailnum = ixnum;
        ...
        if (iq->debug) {
            reqprintf(iq, "ix_addk IX %d LEN %u KEY ", ixnum, keysize);
            reqdumphex(iq, key, keysize);
            reqmoref(iq, " RC %d", rc);
        }
               ...
               int flags)
{
    ...
    *ixfailnum = -1;
    ...
    retrc = upd_record_indices(iq, trans, 
        ...
        od_dta, od_len, 
        old_dta, del_keys, flags, 
        add_idx_blobs, del_idx_blobs, 
